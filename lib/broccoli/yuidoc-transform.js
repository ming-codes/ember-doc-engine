...
Transform.prototype.transform = function() {
  ...
  var hash = paths.reduce(function(accum, relativePath) {
    var doc = serializer.parse(fs.readFileSync(path.join(rootPath, relativePath), 'utf8'));
...
Transform.prototype.writeJSON = function(json) {
  var outputFile = path.join(this.outputPath, this.options.outputFile + '.js');

  fs.writeFileSync(outputFile, serializer.stringify(this.writer.writeJSON(this.parser)));
};
